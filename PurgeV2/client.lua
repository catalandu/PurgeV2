...
function MFP:HandleLegion()
  ...
  if dist < self.LegionSpawnDist then
    if self.LegionSpawned then 
      ...
      if closest and (not self.AmmoLooted or not self.AmmoLooted[closest]) and dist < self.DrawTextDist then
        Utils:DrawText3D(closest.x,closest.y,closest.z+1.0, "Press [~r~E~s~] to loot.")
...
function MFP:LootAmmo(box)
  ...
  exports["taskbar"]:taskBar(self.InteractTimer * 1000, "Looting")
  canLoop = false
  if doCont then
